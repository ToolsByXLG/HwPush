...
namespace HwPush.HwBase
{
    public class HTMLHelper
    {
        ...
        private static WebProxy pxy = new WebProxy();
        #endregion
        ...
        public static string GetHtml(string url, string postData, bool isPost, string cookie)
        {
            if (string.IsNullOrEmpty(postData)) return GetHtml(url, cookie);
            ...
            try
            {
                ...
                httpWebRequest.Proxy = pxy;


                httpWebRequest.Headers[HttpRequestHeader.Cookie] = cookie;
            ...
            {
                if (currentTry <= maxTry) GetHtml(url, postData, isPost, cookie);
                currentTry--;
        ...
        public static string GetHtml(string url, string cookie)
        {
            ...
            {
                if (currentTry <= maxTry) GetHtml(url, cookie);
                currentTry--;
                if (httpWebRequest != null) httpWebRequest.Abort();
                if (httpWebResponse != null) httpWebResponse.Close();
                return string.Empty;
        ...
        /// <param name="url">地址</param>
        /// <param name="cookieContainer">cookieContainer</param>
        public static Stream GetStream(string url, string cookie)
        {
            currentTry++;
        ...
        public static string Post_Http(string url, string postData = "")
        {
            ...
            try
            {
...
#if !DEBUG
                //WebProxy proxyObject = new WebProxy("127.0.0.1", 8877);//代理设置
                WebProxy proxyObject = new WebProxy();//代理设置
                ...
                if (!string.IsNullOrEmpty(PublicClass.GlobalPublicKey))
                {
                    myRequest.Headers.Add("QQ", RsaHelper.RSACrypto.Encrypt(PublicClass.GlobalPublicKey, PublicClass.GlobalQQNumber.ToString()));
                    if (!string.IsNullOrEmpty(Ver))
                    {
                        myRequest.Headers.Add("Ver", RsaHelper.RSACrypto.Encrypt(PublicClass.GlobalPublicKey, Ver));
                    }
                }
                else
                {
                    myRequest.Headers.Add("QQ", "");
                    if (!string.IsNullOrEmpty(Ver))
                    {
                        myRequest.Headers.Add("Ver", Ver);
                    }
                }
